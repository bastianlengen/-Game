    ...
    def setup(self, tmx_map, overworld_frames):
        ...
        for obj in tmx_map.get_layer_by_name('Objects'):
            if obj.name == 'palm':
                ...
                AnimatedSprite((obj.x, obj.y), overworld_frames['palm'], self.all_sprites, Z_LAYERS['main'], animation_speed)
            else:
        ...
        for obj in tmx_map.get_layer_by_name('Paths'):
            pos_list = [(int(p.x + TILE_SIZE / 2), int(p.y + TILE_SIZE / 2)) for p in obj.points]
            start = obj.properties['start']
            end = obj.properties['end']
            self.paths[end] = {'pos': pos_list, 'start': start}
        ...
        for obj in tmx_map.get_layer_by_name('Nodes'):
            ...
            if obj.name == 'Node' and obj.properties['stage'] == self.data.current_level:
                self.icon = Icon(pos = (obj.x + TILE_SIZE / 2, obj.y + TILE_SIZE / 2),
